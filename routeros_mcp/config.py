...
This module provides a Pydantic-based Settings class for application configuration
with support for environment variables, YAML/TOML config files, and CLI overrides.
...
class Settings(BaseSettings):
    ...
    mcp_http_host: str = Field(
        default="127.0.0.1", description="HTTP server bind address (0.0.0.0 for all interfaces)"
    )
    ...
    encryption_key: str | None = Field(
        default=None, description="Master encryption key for secrets (base64, 32+ bytes)"
    )
