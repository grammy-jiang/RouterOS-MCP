...
async def http_server(test_settings: Settings) -> AsyncMock:
    ...
    server.base_url = f"http://{test_settings.mcp_http_host}:{test_settings.mcp_http_port}{test_settings.mcp_http_base_path}"
    return server


@pytest.mark.asyncio
@pytest.mark.skip(
    reason="Full HTTP server not yet exposed - will be completed in Phase 3"
)
async def test_mcp_client_basic_tool_call(http_server: AsyncMock) -> None:
    ...
    async with sse_client(url=base_url) as (read_stream, write_stream):
        async with ClientSession(
            ...
            device_list_tool = next(
                (t for t in tools_result.tools if t.name == "device_list"), None
            )
    ...
    async with sse_client(url=base_url) as (read_stream, write_stream):
        async with ClientSession(
            ...
            result = await session.call_tool(
                name="device_get",
                arguments={"device_id": "dev-test-001"}
            )
...
async def test_mcp_client_error_handling_invalid_device(
    ...
    async with sse_client(url=base_url) as (read_stream, write_stream):
        async with ClientSession(
            ...
            result = await session.call_tool(
                name="device_get",
                arguments={"device_id": "non-existent-device"}
            )
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="OIDC integration not yet complete - will be completed in Phase 3"
)
...
async def test_http_client_direct_jsonrpc_request() -> None:
    ...
    # Process request
    with patch("routeros_mcp.mcp.transport.http_sse.get_correlation_id", return_value="test-corr-id"):
...
async def test_correlation_id_propagation() -> None:
    ...
    with patch("routeros_mcp.mcp.transport.http_sse.get_correlation_id", return_value=custom_corr_id):
        response = await transport.handle_request(mock_request)
...
async def test_concurrent_client_requests() -> None:
    ...
    async def make_request(request_id: str) -> dict[str, Any]:
        ...
        with patch("routeros_mcp.mcp.transport.http_sse.get_correlation_id", return_value=f"corr-{request_id}"):
            response = await transport.handle_request(mock_request)
...
async def test_malformed_json_handling() -> None:
    ...
    with patch("routeros_mcp.mcp.transport.http_sse.get_correlation_id", return_value="malformed-test"):
        response = await transport.handle_request(mock_request)
