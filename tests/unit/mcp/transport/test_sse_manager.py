...
async def test_concurrent_subscriptions() -> None:
    ...
    tasks = [
        manager.subscribe(f"client-{i}", f"device://dev-{i % 3}/health")
        for i in range(10)
    ]
...
async def test_multiple_resources_per_client() -> None:
    ...
    sub2 = await manager.subscribe(client_id, "device://dev-001/config")
    sub3 = await manager.subscribe(client_id, "device://dev-002/health")
