...
**Phase 1 Scope**: Single-user, stdio-only MCP server with OS-level access control. Multi-user OAuth/OIDC authentication is implemented in Phase 4.
...
- **MCP Service (This Application)**
  ...
  - **Phase 4**: OAuth/OIDC for multi-user access
...
These features are planned for **Phase 4: Enterprise Multi-User**.
...
## Future: Multi-User Support (Phase 4)

**Phase 1 is designed to evolve into multi-user without breaking changes:**
...
❌ **RBAC** (Phase 4 - device capability flags only in Phase 1)
...
**Phase 1 provides production-ready security for single-user deployments while preserving a clear migration path to enterprise multi-user in Phase 4.**
